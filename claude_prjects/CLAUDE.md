...
日本語の文章（スライド・ブログ・ドキュメント）を生成する前に、
`claude_prjects/Obsidian/Documents/Obsidian/Clippings/日本語表現の好み.md` を読み、
場面が合致するルールを適用する。セッション内で最初の1回読めばよい。

このファイルは crit レビューでの指摘から育てる。運用手順は /review スキルを参照。
